...
export function nameToStateTag(name: string): string {
    return shipState[name] || '';
}

export function stateTagToName(tag: string): string {
    return Object.keys(shipState).find(name => shipState[name] === tag);
}
